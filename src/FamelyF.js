...
function FamelyF(props) {
  ...
  const [storageFonts, setStorageFonts] = useState({ key: "fonts", content: "" });
  ...
  return (
    <div className="col-sm-2 form-group text-left p-0 fontsFm">
      ...
      {dispPan === true ?
        <table style={STYLES.table}>
          <tbody>
            <tr>
              <td colSpan="2">
                <textarea rows="5" cols="20" defaultValue={replaceFontsTextarera(storObj)} onChange={(e) => setStorageFonts({ key: "fonts", content: e.target.value })} />
              </td>
            </tr>
            <tr>
              <td>
                <div style={STYLES.save}>
                  {disp === true ? <button className={"btn btn-primary"} type="button" onClick={() => {
      ...
      <ul id="fontsTxtFamely">
        <li>{nf}
          <ul>
            {n.map((x, i) => <li key={x + i} onClick={() => { props.fontsFm(props, x); setnf(x); props.setActive((props.active === false) ? true : false); props.setSelectPanelDicplay(false); }}>{x}</li>)}
          </ul>
        </li>
      </ul>
    </div>
