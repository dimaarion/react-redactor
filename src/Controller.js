...
function Controller(props) {
    ...
    function types(
        ...
        if (selectedtext.length > 0) {
            ititalTegs()
                .filter((f, i) => i === items)
                .map((x, i) => x.innerHTML = x.innerHTML.replace(selectedtext, "<" + teg + ">" + selectedtext + "</" + teg + ">"));
        }
    }
    ...
    function imgWidthR(ititalTegs, imgurls, imgWidth) {
        ititalTegs().map((x) =>
            Object.values(x.getElementsByTagName('img'))
                .filter((l) => '/' + l.src.split('/')
                    .filter((f, i) => i > 2).join('/') === imgurls)
    ...
    function clicEvEl(setgTags) {
        ['div', 'h1', 'img', 'h2', 'h3', 'b', 'i', 'u', 'ul', 'li', 'ol', 's', 'em', 'strong', 'p'].map((x) => Array
            .from(document
                .getElementsByClassName(baseSelector))
            .map((x2) => Array
                .from(x2.getElementsByTagName(x)).map((x3) => x3
            ...
            Object.values(x.getElementsByTagName('img')).map(
                (el, ix) =>
                    (el.onclick = function (e) {
                        ...
                        seteX(e.x);
                        seteY(e.y);
                        setimgurls('/' + e.target.src.split('/').filter((f, i) => i > 2).join('/'));
                        e.target.src = '/' + e.target.src.split('/').filter((f, i) => i > 2).join('/')
    ...
    return (
        <div className="contentDtext">
            <div className={sizes === true ? "cintent_text" : "cintent_text_full"}>
                <div className="row container text-right p-4  panel" style={(sizes === true) ? { position: 'absolute' } : panelStyle}>
